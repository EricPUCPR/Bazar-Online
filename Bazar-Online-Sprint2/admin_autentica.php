...
$stmt = $conn->prepare("SELECT id, nome, email, email_verificado, is_admin FROM usuarios WHERE email = ? AND is_admin = 1 LIMIT 1");
...
try {
    ...
    echo json_encode([
        "success" => false,
        "mensagem" => "Não foi possível enviar o código de login admin."
    ]);
    exit;
}
...
echo json_encode([
    "success" => true,
    "mensagem" => "Código enviado para o e-mail admin."
]);
?>
